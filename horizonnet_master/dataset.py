...
from horizonnet_master.misc import panostretch
...
class PanoCorBonDataset(data.Dataset):
    ...
    def __init__(self, root_dir,
                 ...
                 normcor=False, return_cor=False, return_path=False, val=False, use_line=True):
        self.val = val
        self.use_line = use_line
        self.line_dir = os.path.join(root_dir, 'line_img')
        ...
        self.line_fnames = ['%s_aligned_line.png' % fname[:-4] for fname in self.img_fnames]
        self.flip = flip
        self.rotate = rotate
        self.gamma = gamma
    ...
    def __getitem__(self, idx):
        ...
        H, W = img.shape[:2]
        # Read line image
        if self.use_line == True:
            line_img_path = os.path.join(self.line_dir, self.line_fnames[idx])
            line = np.array(Image.open(line_img_path), np.float32)[..., :3] / 255.
        ...
        # x = np.concatenate((img.copy(),line.copy()),axis=2)

        x = img.copy()
        x = torch.FloatTensor(x.transpose([2, 0, 1]))
        if self.use_line == True:
            line = torch.FloatTensor(line.copy().transpose([2, 0, 1]))
        bon = torch.FloatTensor(bon.copy())
        y_cor = torch.FloatTensor(y_cor.copy())

        # Check whether additional output are requested
        if self.use_line == True:
            out_lst = [x, line, bon, y_cor]
        else:
            out_lst = [x, bon, y_cor]
        if self.return_cor:
            out_lst.append(cor)
