...
from horizonnet_master.model import HorizonNet
from horizonnet_master.dataset import visualize_a_data
from horizonnet_master.misc import post_proc, panostretch, utils
...
def inference(net, x, line, device, flip=False, rotate=[], visualize=False,
              force_cuboid=True, min_v=None, r=0.05, type='line'):
    ...
    if line!= None:
        y_bon_, y_cor_ = net(x.to(device), line.to(device))
    else :
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        for i_path in tqdm(paths, desc='Inferencing'):
            ...
            if args.type == 'line':
                line=Image.open((i_path.split('img')[0]+'line_img'+i_path.split('img')[1]).split('.')[0]+'_aligned_line.png')
            if img_pil.size != (1024, 512):
                img_pil = img_pil.resize((1024, 512), Image.BICUBIC)
            img_ori = np.array(img_pil)[..., :3].transpose([2, 0, 1]).copy()
            line = np.array(line)[..., :3].transpose([2, 0, 1]).copy()
            if args.type == 'line':
                line_ori = np.array(line)[..., :3].transpose([2, 0, 1]).copy()

            x = torch.FloatTensor([img_ori / 255])
            line = torch.FloatTensor([line / 255])
            # Inferenceing corners
            cor_id, z0, z1, vis_out = inference(net, x, line, device,
                                                args.flip, args.rotate,
                                                args.visualize,
                                                args.force_cuboid,
                                                args.min_v, args.r,args.type)
